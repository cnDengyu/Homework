...
int main()
{
	...
	matA = MatrixBuildFromArray(&dataA[0][0], SCALE, SCALE);
	...
	// 叉乘测试
	puts("叉乘测试");
	vec1 = MatrixBuildFromArray(v1, 3, 1);
	vec2 = MatrixBuildFromArray(v2, 3, 1);
	MatrixCrossProduct(vec1, vec2, &matResult);
	MatrixPrint(matResult);
